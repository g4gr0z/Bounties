...
contract BountyManager {
    ...
    event BountyFailed(uint bountyIndex);
    ...
    function createBounty(
        ...
        newBounty.state = State.Open;

        emit BountyCreated(msg.sender, _description, _reward, _deadline);
    }

    function markBountyClosed(uint bountyIndex) public {
        bountyList[bountyIndex].state = State.Closed;
        emit BountyFailed(bountyIndex);
    }
    ...
    function closeBountyIfDeadlineReached(uint bountyIndex) public {
        ...
        require(
            bounty.deadline < block.timestamp,
    ...
    function bountySubmission(
        ...
        require(
            bounty.deadline > block.timestamp,
            "Deadline has not passed yet."
        ); //this will ensure submissions occur before deadline
    }

    //returns number of submissions made for any bounty
    function getNumSubmissionsForBounty(
        uint bountyIndex
    ...
    function selectBountyWinner(
        uint bountyIndex,
        uint[] memory _submissionIndexes
    ) public {
        require(bountyList[bountyIndex].owner == msg.sender, "Not authorised");
        ...
        require(
            bounty.deadline < block.timestamp,
            "Deadline has not passed yet."
        ); //this will ensure winner is selected after deadline

        for (uint i = 0; i < _submissionIndexes.length; i++) {
            uint submissionIndex = _submissionIndexes[i];
            require(
                submissionIndex < bounty.numSubmission,
                "Invalid submission index."
            );
            require(
                !bounty.submissionList[submissionIndex].approved,
                "This submission has already been approved."
            );
            bounty.submissionList[submissionIndex].approved = true;
            payable(bounty.submissionList[submissionIndex].submitter).transfer(
                bounty.reward / _submissionIndexes.length
            );
            emit WinnerSelected(
                bounty.submissionList[submissionIndex].submitter,
                bounty.reward / _submissionIndexes.length
            );
        }
    }
    ...
    function getBountySubmissionByIndex(
        uint bountyIndex,
